      ...
      <section className="c-banner c-banner--home">
        <div className="l-container">
          <div className="c-banner__content">
            ...
            <span className="c-banner__breadcrumbs">
              Home / Blog / <span className="c-banner__breadcrumbs--current">{post_title[0].text}</span>
            </span>
          </div>
        </div>
        ...
        {body.map((block, index) => {
          switch (block.slice_type) {
            case "quote_block":
              return (
                <section
                  className="p-blogPost__block p-blogPost__block--fullMobile"
                  key={`${block.slice_type}-${index}`}
                >
                  <div className="l-container">
                    <div className="p-blogPost__quote">
                      <h2 className="p-blogPost__quoteHeading">{block.primary.quote}</h2>
                      <span className="p-blogPost__quoteAuthor">
                        {block.primary.author}, {block.primary.authors_title}
                      </span>
                    </div>
                  </div>
                </section>
            ...
            default:
              return (
                <section className="p-blogPost__block" key={`${block.slice_type}-${index}`}>
                  <div className="l-container">
                    <div className="p-blogPost__copy">{RichText.render(block.primary.content_block)}</div>
                  </div>
                </section>
              )
          }
